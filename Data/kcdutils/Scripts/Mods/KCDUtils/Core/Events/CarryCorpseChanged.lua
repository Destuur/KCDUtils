...
KCDUtils.Events.CarryCorpseChanged = KCDUtils.Events.CarryCorpseChanged or {}

local CSC = KCDUtils.Events.CarryCorpseChanged
...
function CSC.startUpdater()
    local fn = function(deltaTime)
        ...
        local isCarryingCorpse = player.actor:IsCarryingCorpse()
        if isCarryingCorpse == nil then return end

        for i = #CSC.listeners, 1, -1 do
            local sub = CSC.listeners[i]

            if sub.isPaused then
                sub.pausedState = isCarryingCorpse
            else
                local effectiveLastState = sub.pausedState or sub.lastState

                if effectiveLastState ~= nil and isCarryingCorpse ~= effectiveLastState then
                    local shouldTrigger = false
                    if sub.trigger == "both" then
                        shouldTrigger = true
                    elseif sub.trigger == "entry" and isCarryingCorpse then
                        shouldTrigger = true
                    elseif sub.trigger == "exit" and not isCarryingCorpse then
                        shouldTrigger = true
                    end
                ...
                sub.lastState = isCarryingCorpse
                sub.pausedState = nil
            end
        end
    end
...
--- CarryCorpseChanged Event
--- Fires when the player's corpse carrying state changes
---
...
function KCDUtils.Events.CarryCorpseChanged.Add(config, callback)
    return addListener(config, callback)
end

--- Remove a previously registered subscription
--- @param subscription table The subscription object returned from Add()
function KCDUtils.Events.CarryCorpseChanged.Remove(subscription)
    return removeListener(subscription)
end

--- Pause a subscription without removing it
--- @param subscription table The subscription object returned from Add()
function KCDUtils.Events.CarryCorpseChanged.Pause(subscription)
    if subscription then subscription.isPaused = true end
end

--- Resume a paused subscription
--- @param subscription table The subscription object returned from Add()
function KCDUtils.Events.CarryCorpseChanged.Resume(subscription)
